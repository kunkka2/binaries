...
// ProxyConfig holds SOCKS5 proxy details
...
// proxyConfig is a package-level variable for SOCKS5 proxy configuration
...
func ParseSOCKS5Proxy(proxyURL string) (*ProxyConfig, error) {
    if !strings.HasPrefix(proxyURL, "socks5://") {
        return nil, fmt.Errorf("invalid proxy scheme, expected 'socks5://'")
    }
    ...
    port := u.Port()
    if port == "" {
        return nil, fmt.Errorf("port is missing in proxy URL")
    }
...
func dialSOCKS5(network, addr string, timeout time.Duration,localAddr *net.Addr,) (net.Conn, error) {
    ...
    if proxyConfig == nil {
        ...
        if err != nil {
            return nil, fmt.Errorf("direct dial failed: %v", err)
        }
        return conn, nil
    }
    ...
    dialer, err := proxy.SOCKS5("tcp", proxyConfig.Host+":"+proxyConfig.Port, auth, proxy.Direct)
    if err != nil {
    ...
    conn, err := dialer.Dial(network, addr)
    if err != nil {
        return nil, fmt.Errorf("SOCKS5 dial failed: %v", err)
    }

    // Set deadline to enforce timeout
...
func open(address string, localAddr *net.Addr, timeout time.Duration, connect_timeout time.Duration, timeoutMode int,
	...
	c.conn, err = dialSOCKS5("tcp",address,timeout,localAddr)
